...
******************************************************************************/
{
  ...
  if (tiny_radfract == NULL) {
    nrerror((char*) "Memory allocation error in mtclim_init() ...\n");
  }
  for (i=0; i<366; i++) {
  ...
  if (data_free(&ctrl, &mtclim_data)) {
    nrerror((char*) "Error in data_free()... exiting\n");
  }
  for (i=0; i<366; i++) {
		   ...
		   parameter_struct *p, data_struct *mtclim_data)
{
  ...
  if (have_dewpt) {
    if (have_dewpt == 1) {
      nrerror((char*) "have_dewpt not yet implemented for tdew; however you can supply observed vapor pressure and set have_dewpt to 2\n");
    }
  ...
  if (data_alloc(ctrl, mtclim_data)) {
    nrerror((char*) "Error in data_alloc()... exiting\n");
  }
